...
from extractor.page_rank.sm_mobile_extractor import SmMobileExtractor
from download_center.new_spider.util.util_md5 import UtilMD5

class SmMobileSpider(BaseRankSpider):
    """
        神马移动排名
    """
    def __init__(self):
        super(SmMobileSpider, self).__init__()
        self.search_device = DeviceEnums.sm_mobile
        self.extractor = SmMobileExtractor()

    def get_request_url(self, keyword, pn):
        kwh = {'q': keyword, "from": "smor", "safe": 1, "by": "submit", "snum": 0}
        if pn > 1:
            url = 'https://m.sm.cn/s?' + urllib.urlencode(kwh) + "&page={}".format(pn)
        else:
            url = 'https://m.sm.cn/s?' + urllib.urlencode(kwh)
        return url
