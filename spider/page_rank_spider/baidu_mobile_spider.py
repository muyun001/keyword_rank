...
from extractor.page_rank.baidu_mobile_extractor import BaiduMobileExtractor
from download_center.new_spider.util.util_md5 import UtilMD5

class BaiduMobileSpider(BaseRankSpider):
    """
       百度移动排名
    """
    def __init__(self):
        super(BaiduMobileSpider, self).__init__()
        self.search_device = DeviceEnums.baidu_mobile
        self.extractor = BaiduMobileExtractor()

    def get_request_url(self, keyword, pnum):
        kwh = {'wd': keyword}
        if pnum != 1:
            kwh['pn'] = (pnum - 1) * 10
    ...
    def get_request_param(self, task_id, keyword, target_url, page, spidertype, keyword_id, site_name, pnum=1):
         ...
         configs = {"store_type": 2, "param": {"capture_width": 414, "capture_height": 736}}
         if pnum != 1:
             configs['priority'] = 3

         pc_id_build = Baidu_id_build()
         headers = {
             'User-Agent': UserAgentUtil().random_one(self.search_device),
             'Accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3',
             'Cookie': """BAIDUID={}""".format(pc_id_build.build_baiduid())
         }
