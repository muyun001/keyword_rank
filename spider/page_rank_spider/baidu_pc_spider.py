...
from extractor.page_rank.baidu_pc_extractor import BaiduPCExtractor
from download_center.new_spider.util.util_md5 import UtilMD5

class BaiduPCSpider(BaseRankSpider):
    """
       百度PC排名
    """
    def __init__(self):
        super(BaiduPCSpider, self).__init__()
        self.search_device = DeviceEnums.baidu_pc
        self.extractor = BaiduPCExtractor()
    ...
    def get_request_param(self, task_id, keyword, target_url, page, spidertype, keyword_id, site_name, pnum=1):
         ...
         configs = {"store_type": 2}
         if pnum != 1:
             configs['priority'] = 3

         pc_id_build = Baidu_id_build()
         headers = {'User-Agent': UserAgentUtil().random_one(self.search_device),"Cookie": """BAIDUID={}""".format(pc_id_build.build_baiduid())}
         urls = [{
